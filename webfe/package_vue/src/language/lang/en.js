...
export default {
  ...
  '格式不正确，由小写字母组成，长度小于 20 个字符': 'Incorrect format, consisting of lowercase letters, less than 20 characters long',
  '由小写字母组成，长度小于 20 个字符': 'Consists of lowercase letters and is less than 16 characters long',
  '蓝鲸可视化开发平台的应用 ID 只能由小写字母组成, 所属应用': 'The APP ID of the BlueKing visualization development platform can only be composed of lowercase letters, and the APP belongs to',
  '的应用 ID 为': 'APP ID is',
  '故无法在当前应用下创建蓝鲸可视化开发平台的模块。': 'Therefore, it is impossible to create modules of the BlueKing visualization development platform under the current APP.',
  构建方式: 'Deploy methods',
  ...
  可视化开发: 'Lesscode',
  '蓝鲸智云可视化开发平台提供了前端页面在线可视化拖拽组装、配置编辑、源码生成、二次开发等能力。旨在帮助用户通过尽量少的手写代码的方式快速设计和开发 SaaS': 'The BlueKing Lesscode provides online visual assembly of front-end pages, configuration editing, source code generation, customization development and other capabilities.',
  ...
  验证查看: 'Verify to view',
  点击查看: 'Click to view',
  '已部署密钥概览：': 'Overview of deployed keys:',
  模块: 'Module',
  环境: 'Environment',
  ...
  禁用密钥: 'Disable key',
  '禁用此密钥后，蓝鲸云 API 将拒绝此密钥的所有请求。禁用后，预计 15 分钟内生效。': 'After disabling this key, the BlueWhale Cloud API will reject all requests for this key. Once disabled, it is expected to take effect within 15 minutes.',
  删除密钥: 'delete key',
  ...
  '密钥已达到上限，应用仅允许有 2 个密钥': 'Key limit reached, app is only allowed 2 keys',
  ...
  '请输入 1 - 65535 的整数，非必填': 'Please enter an integer from 1 - 65535, not required',
  请输入带标签的镜像仓库: 'Please enter the tagged image repository',
  确认删除该进程: 'Confirm to delete this process',
  '鲸可视化开发平台的应用 ID 只能由小写字母组成, 所属应用': 'The application ID of the Whale Visualization Development Platform can only consist of lowercase letters, belonging to the application',
  '故无法在当前应用下创建蓝鲸可视化开发平台的模块。': 'So it is impossible to create a module of the Blue Whale Visualization Development Platform under the current application.',
